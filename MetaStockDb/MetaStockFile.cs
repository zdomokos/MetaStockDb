		...
		public override void Load(string fileName)
		{
			...
			try
			{
				...
				for (UInt32 i = 0; i < Header.num-1; i++)
				{
					...
					tmp = br.ReadUInt32(); MsFileIO.ConvertToIeeeFloat(tmp, ref f);rec.Datef   = f;
					tmp = br.ReadUInt32(); MsFileIO.ConvertToIeeeFloat(tmp, ref f);rec.Open    = f;
					tmp = br.ReadUInt32(); MsFileIO.ConvertToIeeeFloat(tmp, ref f);rec.High    = f;
					tmp = br.ReadUInt32(); MsFileIO.ConvertToIeeeFloat(tmp, ref f);rec.Low     = f;
					tmp = br.ReadUInt32(); MsFileIO.ConvertToIeeeFloat(tmp, ref f);rec.Close   = f;
					tmp = br.ReadUInt32(); MsFileIO.ConvertToIeeeFloat(tmp, ref f);rec.Volume  = f;
					tmp = br.ReadUInt32(); MsFileIO.ConvertToIeeeFloat(tmp, ref f);rec.Unadj   = f;
